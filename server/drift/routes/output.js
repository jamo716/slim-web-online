...
import Drift_Output from "../models/output.js"
import Drift_Paramset from "../models/paramset.js"
...
router.get("/:userid", (req, res) => {
  try{
    Drift_Output.find({userID: parseInt(req.params.userid)})
      .then(outputs => {
...
router.get("/:userid/:id/:run", (req, res) => {
    try {
      Drift_Output.find({userID: parseInt(req.params.userid), id: parseInt(req.params.id), run: parseInt(req.params.run)})
        .then(output => {
...
router.post("/:id", async (req, res) => {
    ...
    const outputs = await Drift_Output.find({id: parseInt(req.params.id)})
    ...
    let paramSetToRun = await Drift_Paramset.find({id: parseInt(req.params.id)})
    ...
    const child = spawn("slim", ["-d", `popSize=${paramSetToRun.popSize}`, "-d", `id=${req.params.id}`, "-d", `simLength=${paramSetToRun.simLength}`, "server/drift/slim-scripts/drift.slim"])
    ...
    child.on("exit", (code, signal) => {
      ...
      fs.createReadStream(`server/drift/slim-output/${parseInt(req.params.id)}/output.csv`)
      .pipe(csv.parse({}))
      .on("data", (data) => {
        const generationOutput = 
        {
          generation: parseInt(data[0]),
          fExp: parseFloat(data[1]),
          fEst: parseFloat(data[2]),
          alleleFreq: parseFloat(data[3])
        }
        fileOutputs.push(generationOutput)
      })
      .on("end", () => {
        //format for new output object that we save to the database
        const newOutput = new Drift_Output({
        ...
        const path = `server/drift/slim-output/${parseInt(req.params.id)}`
  ...
  router.delete("/:userid/:id/:run", (req, res) => {
    try {
      Drift_Output.findOneAndDelete({userID: parseInt(req.params.userid), id: parseInt(req.params.id), run: parseInt(req.params.run)})
        .then(output => {
          res.status(200).json(output)
        })
    } catch (error) {
