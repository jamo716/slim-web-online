...
router.get("/:userid", (req, res) => {
    try {
      Mut_Sel_Paramset.find({userID: parseInt(req.params.userid)})
        .then(paramsets => {
...
router.delete("/:id", (req, res) => {
  try {
    Mut_Sel_Paramset.findOneAndDelete({id: parseInt(req.params.id)})
      .then(paramset => {
        res.status(200).json(paramset)
      })
  } catch (error) {
