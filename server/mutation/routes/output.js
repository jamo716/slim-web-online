...
import Mut_Output from "../models/output.js"
...
router.get("/:userid", (req, res) => {
  try{
    Mut_Output.find({userID: parseInt(req.params.userid)})
      .then(outputs => {
...
router.get("/:userid/:id/:run", (req, res) => {
    try {
      Mut_Output.find({userID: parseInt(req.params.userid), id: parseInt(req.params.id), run: parseInt(req.params.run)})
        .then(output => {
...
router.post("/:id", async (req, res) => {
    ...
    const outputs = await Mut_Output.find({id: parseInt(req.params.id)})
    ...
    let paramSetToRun = await Mut_Paramset.find({id: parseInt(req.params.id)})
    ...
    console.log(paramSetToRun.id)
  
    //spawn a child process to run slim script with specified parameters
    const child = spawn("slim", ["-d", `fr=${paramSetToRun.fr}`, "-d", `br=${paramSetToRun.br}`, "-d", `pInit=${paramSetToRun.pInit}`, "-d", `id=${req.params.id}`, "-d", `simLength=${paramSetToRun.simLength}`, "server/mutation/slim-scripts/mutation.slim"])
    ...
    child.on("exit", (code, signal) => {
      ...
      fs.createReadStream(`server/mutation/slim-output/${parseInt(req.params.id)}/output.csv`)
      .pipe(csv.parse({}))
      .on("data", (data) => {
        console.log(data)
      ...
      .on("end", () => {
        ...
        const newOutput = new Mut_Output({
        ...
        const path = `server/mutation/slim-output/${parseInt(req.params.id)}`
  ...
  router.delete("/:userid/:id/:run", (req, res) => {
    try {
      Mut_Output.findOneAndDelete({userID: parseInt(req.params.userid), id: parseInt(req.params.id), run: parseInt(req.params.run)})
        .then(output => {
          res.status(200).json(output)
        })
    } catch (error) {
