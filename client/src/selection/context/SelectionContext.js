...
export const SelectionProvider = ({children}) => {
    ...
    useEffect(() => {
        ...
        axios.get(`/api/selection/paramset/${userID}`).then((paramSets) => {
            setParamSets(paramSets.data)
        })
    
        axios.get(`/api/selection/output/${userID}`).then((serverOutputs) => {
    ...
    const addParamSet = (paramSet) => {
        ...
        axios.post("/api/selection/paramset", newParamSet)
        setParamSets([...paramSets, newParamSet])
    }
    
    //deletes a parameter set
    const deleteParamSet = (id) => {
      axios.delete(`/api/selection/paramset/${id}`)
    ...
    async function receiveOutput() {
      try {
        const userID = Cookies.get("userID")
        const response = await axios.get(`/api/selection/output/${userID}`);
    ...
    const renderParameterSet = async (id) => {
      ...
      await axios.post(`/api/selection/output/${id}`)
    ...
    const deleteOutput = async (id, run) => {
      const userID = Cookies.get("userID")
      await axios.delete(`/api/selection/output/${userID}/${id}/${run}`)
    ...
    const renderGraph = async (id, run) => {
      try{
        ...
        const response = await axios.get(`/api/selection/output/${userID}/${id}/${run}`)
