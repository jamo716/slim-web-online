...
export const MutationProvider = ({children}) => {
    ...
    useEffect(() => {
        ...
        axios.get(`/api/mutation/paramset/${userID}`).then((paramSets) => {
            setParamSets(paramSets.data)
        })
    
        axios.get(`/api/mutation/output/${userID}`).then((serverOutputs) => {
    ...
    const addParamSet = (paramSet) => {
        ...
        axios.post("/api/mutation/paramset", newParamSet)
        setParamSets([...paramSets, newParamSet])
    }
    
    //deletes a parameter set
    const deleteParamSet = (id) => {
      axios.delete(`/api/mutation/paramset/${id}`)
    ...
    async function receiveOutput() {
      try {
        const userID = Cookies.get("userID")
        const response = await axios.get(`/api/mutation/output/${userID}`);
    ...
    const renderParameterSet = async (id) => {
      ...
      await axios.post(`/api/mutation/output/${id}`)
    ...
    const deleteOutput = async (id, run) => {
      const userID = Cookies.get("userID")
      await axios.delete(`/api/mutation/output/${userID}/${id}/${run}`)
    ...
    const renderGraph = async (id, run) => {
      try{
        ...
        const response = await axios.get(`/api/mutation/output/${userID}/${id}/${run}`)
