...
export const MutationSelectionContext = createContext()

export const MutationSelectionProvider = ({children}) => {
    ...
    useEffect(() => {
        ...
        axios.get(`/api/mutationselection/paramset/${userID}`).then((paramSets) => {
            setParamSets(paramSets.data)
        })
    
        axios.get(`/api/mutationselection/output/${userID}`).then((serverOutputs) => {
    ...
    const addParamSet = (paramSet) => {
        ...
        axios.post("/api/mutationselection/paramset", newParamSet)
        setParamSets([...paramSets, newParamSet])
    }
    
    //deletes a parameter set
    const deleteParamSet = (id) => {
      axios.delete(`/api/mutationselection/paramset/${id}`)
    ...
    async function receiveOutput() {
      try {
        const userID = Cookies.get("userID")
        const response = await axios.get(`/api/mutationselection/output/${userID}`);
    ...
    const renderParameterSet = async (id) => {
      ...
      await axios.post(`/api/mutationselection/output/${id}`)
    ...
    const deleteOutput = async (id, run) => {
      const userID = Cookies.get("userID")
      await axios.delete(`/api/mutationselection/output/${userID}/${id}/${run}`)
    ...
    const renderGraph = async (id, run) => {
      try{
        ...
        const response = await axios.get(`/api/mutationselection/output/${userID}/${id}/${run}`)
    ...
    return (
        <MutationSelectionContext.Provider value={{
            showParamMenu, 
            paramSets,
            outputs, 
            isRendering,
            ...
            {children}
        </MutationSelectionContext.Provider>
    )
  }